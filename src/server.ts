...
__cm_main().catch(e=>{console.error(e);process.exitCode=1});${background ? '\nsetInterval(()=>{},2147483647);' : ''}
      ...
      if (result.exitCode !== 0) {
        ...
        // Mid-size (5KB-100KB): index into FTS5 as a side effect so a later
        // ctx_search can find it, but still return the raw output inline.
      ...
      // Mid-size (5KB-100KB): index into FTS5 as a side effect so a later
      // ctx_search can find it, but still return the raw stdout inline.
...
// ─────────────────────────────────────────────────────────
// Helper: short deterministic hash for distinct per-call source labels
// (dedup by source: batch:${commands}, execute:${language}:${hash} etc. —
// without a distinct suffix, every no-intent exec would clobber the last).
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// Helper: index content into FTS5 as a side effect, without changing
// what gets returned to the caller (mid-size 5KB-100KB auto-index).
// ─────────────────────────────────────────────────────────
