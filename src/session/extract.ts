 ...
 * input shape.
 *
 * The v2 bus has no `message.updated` event, so the v1 parser is unusable
 * here. v2 usage lives under `data` (not `properties.info`); the model is NOT
 * carried on the usage event — it is observed on the matching
 * `session.step.started` and passed in as `modelId` (the caller correlates
 * the two events by `assistantMessageID`); and there is an extra `reasoning`
 * token bucket. Field names verified against the published
 * @opencode/plugin@2.0.16 + @opencode/schema@2.0.16 types.
 *
