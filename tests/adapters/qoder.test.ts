...
describe("QoderAdapter", () => {
  ...
  describe("config templates (configs/qoder/)", () => {
    const configsDir = resolve(process.cwd(), "configs", "qoder");
    ...
    it("all hook commands reference context-mode hook qoder", () => {
      const settings = JSON.parse(
        readFileSync(join(configsDir, "settings.json"), "utf-8"),
      ) as Record<string, unknown>;
      const hooks = settings.hooks as Record<string, Array<Record<string, unknown>>>;
      for (const [eventType, entries] of Object.entries(hooks)) {
        for (const entry of entries) {
          ...
          for (const hook of hookEntries) {
            expect(String(hook.command)).toMatch(/^context-mode hook qoder /);
